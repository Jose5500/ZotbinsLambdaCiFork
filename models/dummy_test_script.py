...
def gen_Sensors(id_dict: dict, session: Session, bin_id: int):
    ...
    for sensor_fun in [gen_WeightSensor, gen_FullnessSensor, gen_UsageSensor]:
        ...
        r_measurement_units = 'mm' if sensor_fun == gen_FullnessSensor else (
            "Number of Trash thrown away within 30 minute interval" if sensor_fun == gen_UsageSensor else "kg")
        r_model = 'Ultrasonic' if sensor_fun == gen_FullnessSensor else (
            "Breakbeam" if sensor_fun == gen_UsageSensor else "Lidar")
        r_make = 'version 1'

        session.add(
            Sensor(id=ran_id, measurement_units=r_measurement_units, model=r_model, make=r_make, waste_bin_id=bin_id))
...
def gen_FullnessMetric(session: Session, sen_id: int, num_metrics: int):
    ...
    previous_metrics = session.query(FullnessMetric).filter_by(sensor_id=sen_id).order_by(
        FullnessMetric.timestamp).all()
...
if __name__ == '__main__':
    ...
    if input(
            'Would you like to create a new bin, which is 3 BinInfo objects and 9 sensor objects? (type yes for yes, anything else for no): ') == "yes":
